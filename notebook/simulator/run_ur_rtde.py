...
import rtde_receive
import sys
sys.path.append("/Users/jangjiyun/Desktop/JY/code/UR5e_simulator")
from onrobot_rg.src.onrobot import RG
from scipy.spatial.transform import Rotation as R
import numpy as np
# UR5 IP 주소 설정
UR5_IP = "192.168.0.251"
RG2_IP = "192.168.0.72"
RG2_PORT = 502
...
async def control_robot(frequency=1):
    ...
    while True:
        ...
        # 로봇 제어 명령 (비동기)
        rtde_c.moveL(
            [-0.4472554498415476, -0.05244588813065923, 0.3360343377116, 2.3014199178195076, 2.133758315088426, -0.06635373277281863], 
            0.1, 0.1
        )
        # 이동 명령 (제어는 안전성을 위해 직접적으로 실행)
        rtde_c.moveL([-0.5072554498415476, -0.05244588813065923, 0.3360343377116, 2.3014199178195076, 2.133758315088426, -0.06635373277281863], 0.1, 0.1)
        await asyncio.sleep(dt)
        rtde_c.moveL([-0.4472554498415476, -0.10244588813065923, 0.3360343377116, 2.3014199178195076, 2.133758315088426, -0.06635373277281863], 0.1, 0.1)
        await asyncio.sleep(dt)
...
async def main():
    ...
    await asyncio.gather(
        receive_robot_state(frequency=50),   # 50Hz로 상태 수신
        # control_robot(frequency=50),           # 50Hz로 제어 명령 전송
